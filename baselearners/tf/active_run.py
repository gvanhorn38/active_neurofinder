...
for i in range(1, 2*active_labels.shape[0]):
  ...
  predictions = base_learner.batch_predict(active_features, verbose=False)
  query = np.argmin(np.abs(predictions[:,0] - predictions[:,1]))
  query_counts[query] += 1

  base_learner.update(active_features[query], active_labels[query], verbose=True)
  ...
  if i % test_increment == 0: 
    ...
    active_prediction = np.argmax(base_learner.batch_predict(active_features[idx], verbose=False), axis=1)
    active_accuracy = metrics.accuracy_score(active_labels[idx], active_prediction)
    active_precision, active_recall, active_f1, support = (tuple(x) for x in metrics.precision_recall_fscore_support(active_labels[idx], active_prediction))
    
    idx = random.sample(range(test_features.shape[0]), 30)
    
    test_prediction = np.argmax(base_learner.batch_predict(test_features[idx], verbose=False), axis=1)
    test_accuracy = metrics.accuracy_score(test_labels[idx], test_prediction)
    test_precision, test_recall, test_f1, support = (tuple(x) for x in metrics.precision_recall_fscore_support(test_labels[idx], test_prediction))
